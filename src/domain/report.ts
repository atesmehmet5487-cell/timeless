...
/** Haftalık/aylık/yıllık dönem için detaylı ödeme listesi. */
export function buildPeriodReport(
  ...
  const occurrences = buildOccurrences(payments, overrides, from, to, todayISO)
    .filter((o) => o.status !== 'skipped')
    .sort((a, b) => a.date.localeCompare(b.date));

  const rows = occurrences.map((o, i) => toRow(o, i, todayISO, categories));

  const totals: Record<Currency, number> = {} as Record<Currency, number>;
  for (const o of occurrences) {
    if (o.status === 'paid') continue;
    const amt = typeof o.payment.amount === 'number' ? o.payment.amount : 0;
    const cur = o.payment.currency;
    totals[cur] = (totals[cur] ?? 0) + amt;
  }

  const periodPart = `${periodLabel} ÖDEME LİSTESİ`.toLocaleUpperCase('tr');
  const name = owner?.trim();
  const heading = name ? `${name.toLocaleUpperCase('tr')} · ${periodPart}` : periodPart;
  ...
  return {
    ...
    hasNote: rows.some((row) => row.note !== null),
    totals,
    footer: `${D.formatShortTR(todayISO)} tarihinde Timeless ile hazırlandı · ${from} – ${to}`,
  };
}
